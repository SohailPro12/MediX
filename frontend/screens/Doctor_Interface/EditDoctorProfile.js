...
export default function EditDoctorProfile() {
  ...
  const reloadDoctorProfile = async () => {
    ...
    if (res.ok) {
      ...
      throw new Error(data.message || t("doctor.editProfile.reloadError"));
    }
  };
  ...
  const pickImage = async () => {
    ...
    if (!perm.granted) {
      Alert.alert(
        t("doctor.editProfile.permission.title"),
        t("doctor.editProfile.permission.message")
      );
      return;
    }
  ...
  const uploadImage = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/doctor/uploadimage/doctor`, {
        method: "POST",
        headers: {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(err.message || t("doctor.editProfile.updateError"));
      }
      await reloadDoctorProfile();
      setIsEditing(false);
      Alert.alert(t("common.success"), t("doctor.editProfile.successMessage"));
    } catch (err) {
  ...
  return (
    <View style={styles.container}>
      <Header name={t("doctor.editProfile.title")} screen="SettingsDScreen" />
          ...
          <View style={styles.headerRow}>
            ...
            <View style={styles.info}>
              ...
              <Text style={styles.specialty}>
                {t("doctor.editProfile.specialist")} {medecin.specialite}
              </Text>
              {isEditing && (
                <TouchableOpacity
                ...
                >
                  <Text style={styles.changeTxt}>
                    {isUploading
                      ? t("doctor.editProfile.uploading")
                      : t("doctor.editProfile.changePhoto")}
                  </Text>
                </TouchableOpacity>
              )}
            </View>
              ...
              <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
                <View>
                  <Text style={styles.section}>
                    {t("doctor.editProfile.about.title")}
                  </Text>
                  {isEditing ? (
                    <TextInput
                      ...
                      placeholder={t("doctor.editProfile.about.placeholder")}
                    />
                  ) : (
                    <Text style={styles.text}>
                      {medecin.description ||
                        t("doctor.editProfile.about.empty")}
                    </Text>
                  )}
                  ...
                  <Text style={styles.section}>
                    {t("doctor.editProfile.education.title")}
                  </Text>
                  {isEditing ? (
                    <>
                      {formation.map((f, i) => (
                        <TextInput
                          ...
                          placeholder={t(
                            "doctor.editProfile.education.placeholder"
                          )}
                        />
                      ))}
                      <TouchableOpacity
                      ...
                      >
                        <Text style={styles.addTxt}>
                          {t("doctor.editProfile.education.add")}
                        </Text>
                      </TouchableOpacity>
                    </>
                  ) : formation.length > 0 ? (
                    ...
                    <Text style={styles.text}>
                      {t("doctor.editProfile.education.empty")}
                    </Text>
                  )}
                  ...
                  <Text style={styles.section}>
                    {t("doctor.editProfile.experience.title")}
                  </Text>
                  {isEditing ? (
                    <>
                      {experience.map((e, i) => (
                        <TextInput
                          ...
                          placeholder={t(
                            "doctor.editProfile.experience.placeholder"
                          )}
                        />
                      ))}
                      <TouchableOpacity
                      ...
                      >
                        <Text style={styles.addTxt}>
                          {t("doctor.editProfile.experience.add")}
                        </Text>
                      </TouchableOpacity>
                    </>
                  ) : experience.length > 0 ? (
                    ...
                    <Text style={styles.text}>
                      {t("doctor.editProfile.experience.empty")}
                    </Text>
                  )}
                </View>
              </TouchableWithoutFeedback>
          ...
          >
            {isEditing ? (
              ...
              <Text style={styles.saveTxt}>
                {t("doctor.editProfile.editButton")}
              </Text>
            )}
          </TouchableOpacity>
        </ScrollView>
