...
const fetchMedecinsBySSO = async (sso) => {
  try {
    const response = await fetch(
      `${API_URL}/api/patient/getAllMedecins/${sso}`
    );
    if (!response.ok) {
...
const App = () => {
  ...
  const categories = [
    {
      id: 1,
      name: t("patient.searchDoctor.categories.all"),
      icon: "stethoscope",
    },
    {
      id: 2,
      name: t("patient.searchDoctor.categories.generalPractitioner"),
      icon: "user-md",
    },
    {
      id: 3,
      name: t("patient.searchDoctor.categories.cardiologist"),
      icon: "heart",
    },
    {
      id: 4,
      name: t("patient.searchDoctor.categories.dentist"),
      icon: "tooth",
    },
    {
      id: 5,
      name: t("patient.searchDoctor.categories.pediatrician"),
      icon: "baby",
    },
    {
      id: 6,
      name: t("patient.searchDoctor.categories.dermatologist"),
      icon: "skin",
    },
    {
      id: 7,
      name: t("patient.searchDoctor.categories.ophthalmologist"),
      icon: "eye",
    },
  ];
  ...
  const filteredDoctors = doctors.filter((doctor) => {
    ...
    const matchesCategory =
      selectedCategory === t("patient.searchDoctor.categories.all") ||
      doctor.specialite.toLowerCase() === selectedCategory.toLowerCase();
